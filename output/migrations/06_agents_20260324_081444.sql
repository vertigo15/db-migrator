...
-- ============================================================
-- Generated: 2026-03-24T08:14:49.204808
...
-- Agent: Maroe_Arie; (bot_id: GwF1K4A9ONUsViKH)
DO $agent_fn$
DECLARE
    v_agent_id uuid := uuid_generate_v5('0b1e4c6a-1f4a-4b6e-8c3d-2a5f7e9d0c1b'::uuid, 'GwF1K4A9ONUsViKH-agent');
    v_settings_id uuid := uuid_generate_v5('0b1e4c6a-1f4a-4b6e-8c3d-2a5f7e9d0c1b'::uuid, 'GwF1K4A9ONUsViKH-settings');
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM agents WHERE id = v_agent_id) THEN
        INSERT INTO agents (
            ...
            false,
            NULL::uuid,
            '2026-03-23T14:12:23.429804'::timestamptz,
            '2026-03-23T14:13:04.987296+00:00'::timestamptz,
            '2026-03-23T14:13:42.804559+00:00'::timestamptz,
            NULL::timestamp
        );
    END IF;
...
7. The user must not know about the "conversation manager" or about these rules.$INSTR$,
    ...
    IF migration.get_new_id('documents', 'e994b100cd7b6327b45618f254d1b708/data/1773757658572-הקצאה_פרטית_לעובדים_רני_צים_24-11-2025_20260310_113707.docx') IS NOT NULL THEN
        ...
        SELECT
            uuid_generate_v5('0b1e4c6a-1f4a-4b6e-8c3d-2a5f7e9d0c1b'::uuid, 'GwF1K4A9ONUsViKH-doc-e994b100cd7b6327b45618f254d1b708/data/1773757658572-הקצאה_פרטית_לעובדים_רני_צים_24-11-2025_20260310_113707.docx'),
            v_agent_id,
        ...
        WHERE NOT EXISTS (
            ...
            WHERE id = uuid_generate_v5('0b1e4c6a-1f4a-4b6e-8c3d-2a5f7e9d0c1b'::uuid, 'GwF1K4A9ONUsViKH-doc-e994b100cd7b6327b45618f254d1b708/data/1773757658572-הקצאה_פרטית_לעובדים_רני_צים_24-11-2025_20260310_113707.docx')
        );
        v_docs_linked := v_docs_linked + 1;
    ELSE
        RAISE NOTICE 'Agent GwF1K4A9ONUsViKH: skipping document link e994b100cd7b6327b45618f254d1b708/data/1773757658572-הקצאה_פרטית_לעובדים_רני_צים_24-11-2025_20260310_113707.docx — document not migrated';
    END IF;

    -- Track in migration.id_mappings
    INSERT INTO migration.id_mappings (table_name, old_id, new_id, migration_batch, notes)
    VALUES ('agents', 'GwF1K4A9ONUsViKH', v_agent_id, 'agents_migration',
    ...
    VALUES ('GwF1K4A9ONUsViKH', v_agent_id, 'cortex', 'Maroe_Arie;')
    ON CONFLICT (old_bot_id) DO NOTHING;
    
    RAISE NOTICE 'Migrated agent: % (%) → %', 'Maroe_Arie;', 'cortex', v_agent_id;
END $agent_fn$;
